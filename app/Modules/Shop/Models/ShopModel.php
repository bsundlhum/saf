...
class ShopModel extends Model
{
    ...
    static public function getShopComment($shopId,$uid)
    {
        $speedScore = $qualityScore = $attitudeScore = 0;
        ...
        $gspeedScore = floatval(number_format($gspeedScore, 1));
        
        $gqualityScore = GoodsCommentModel::whereIn('goods_id', $goodsIds)->avg('quality_score');
        $gqualityScore = floatval(number_format($gqualityScore, 1));
        
        $gattitudeScore = GoodsCommentModel::whereIn('goods_id', $goodsIds)->avg('attitude_score');
        $gattitudeScore = floatval(number_format($gattitudeScore, 1));

        
        
        $employ = EmployModel::where('employee_uid',$uid)->select('id')->get()->toArray();
        $employId = array_flatten($employ);
        $employCommentSpeed = floatval(number_format(EmployCommentsModel::where('to_uid',$uid)->whereIn('employ_id',$employId)->avg('speed_score'),1));
        $employCommentQuality = floatval(number_format(EmployCommentsModel::where('to_uid',$uid)->whereIn('employ_id',$employId)->avg('quality_score'),1));
        $employCommentAtt = floatval(number_format(EmployCommentsModel::where('to_uid',$uid)->whereIn('employ_id',$employId)->avg('attitude_score'),1));
