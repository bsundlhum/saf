...
class GoodsController extends BasicIndexController
{
    ...
    public function pay($id)
    {
        ...
        if (!empty($res) && $uid == $res->uid) {
            if ($res->status == 0) {
                ...
                $shopInfo = ShopModel::find($goods->shop_id);
                $commnetArr = ShopModel::getShopComment($goods->shop_id,$shopInfo['uid']);
                $this->theme->set('SHOPCOMMENT', $commnetArr);
                $this->theme->set('SHOPNAME', $shopInfo['shop_name']);
    ...
    public function confirm($id)
    {
        ...
        if (!empty($orderInfo)) {
            ...
            $shopInfo = ShopModel::find($goodsInfo->shop_id);
            $commnetArr = ShopModel::getShopComment($goodsInfo->shop_id,$shopInfo['uid']);
            $this->theme->set('SHOPCOMMENT', $commnetArr);
            $this->theme->set('SHOPNAME', $shopInfo['shop_name']);
