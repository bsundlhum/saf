...
class IndexController extends BasicIndexController
{
    ...
    public function shop($shopId)
    {
        ...
        if (!empty($shopInfo)) {
            ...
            $commnetArr = ShopModel::getShopComment($shopId,$shopInfo['uid']);
            $this->theme->set('SHOPCOMMENT', $commnetArr);
            $this->theme->set('SHOPNAME', $shopInfo['shop_name']);
    ...
    public function shopabout($shopId)
    {
        ...
        if (!empty($shopInfo)) {
            ...
            $shopInfo['avg_score'] = $commnetArr['avg_score'];
    ...
    public function successstory(Request $request, $shopId)
    {
        $shopId = intval($shopId);
        $this->theme->set('SHOPID', $shopId);
        $shopInfo = ShopModel::where('id', $shopId)->select('uid','shop_name')->first();
        if (!empty($shopInfo)) {
            $this->theme->setUserId($shopInfo['uid']);
        } else {
            abort('404');
    ...
    public function rated(Request $request, $shopId)
    {
        ...
        if (!empty($shopInfo)) {
            ...
            $data = array(
                ...
                'goodsCount' => isset($commnetArr['goodsCount']) ? $commnetArr['goodsCount'] : 0,
                'serviceCount' => isset($commnetArr['serviceCount']) ? $commnetArr['serviceCount'] : 0,
                'speedScore' => isset($commnetArr['speedScore']) ? $commnetArr['speedScore'] : 0,
                'qualityScore' => isset($commnetArr['qualityScore']) ? $commnetArr['qualityScore'] : 0,
                'attitudeScore' => isset($commnetArr['attitudeScore']) ? $commnetArr['attitudeScore'] : 0,
                'shopId' => $shopId,
                'emailStatus' => $emailStatus,
                'isFocus' => $isFocus,
                'contactInfo' => $contactInfo,
                'avg_score' => isset($commnetArr['avg_score']) ? $commnetArr['avg_score'] : 0,
    ...
    public function successDetail($id)
    {
        ...
        $commnetArr = ShopModel::getShopComment($shopId,$successCase->uid);
        $this->theme->set('SHOPCOMMENT', $commnetArr);
        $this->theme->set('SHOPNAME', $shop['shop_name']);
        $data = array(
