...
class IndexController extends \App\Http\Controllers\IndexController
{
    ...
    public function getService(Request $request)
    {
        ...
        $list = UserModel::select('user_detail.sign', 'users.name', 'user_detail.avatar', 'users.id','users.email_status','user_detail.employee_praise_rate','user_detail.shop_status')
        ...
        $shopArr = ShopModel::whereIn('uid',$arrUid)->select('id','uid','is_recommend')->get()->toArray();
        if (!empty($shopArr)) {
            foreach ($list as $key => $item) {
                foreach ($shopArr as $ks => $vs) {
                    if ($item->id == $vs['uid']) {
                        $item->shopId = $vs['id'];
                        $item->is_recommend = $vs['is_recommend'];
                    }
                }
            }
        }
