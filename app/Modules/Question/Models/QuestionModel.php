...
class QuestionModel extends Model
{
    ...
    static public function allQuestion($data)
    {
        ...
        if(!empty($data['time_type']) && $data['time_type']==1)
        {
            if(isset($data['start'])){
                $result = $result->where('question.time','>',date('Y-m-d H:i:s',strtotime($data['start'])));
            }
            if(isset($data['end'])){
                $result = $result->where('question.time','<',date('Y-m-d H:i:s',strtotime($data['end'])));
            }
        }
        
        if(!empty($data['time_type']) && $data['time_type']==2)
        {
            if(isset($data['start'])){
                $result = $result->where('question.verify_at','>',date('Y-m-d H:i:s',strtotime($data['start'])));
            }
            if(isset($data['end'])){
                $result = $result->where('question.verify_at','<',date('Y-m-d H:i:s',strtotime($data['end'])));
            }
        }
