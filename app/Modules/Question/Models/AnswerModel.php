    ...
    static public function adopt($adoptid,$questionid)
    {
        ...
        if(is_null($status))
        {
            ...
            if($question_accept)
            {
                ...
                if($question_accept->is_send_mobile == 1 && $question_accept->code_mobile && $answeruser->mobile){
                    $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                    $templates = [
                        $scheme => $question_accept->code_mobile,
                    ];

                    \SmsClass::sendSms($answeruser->mobile, $templates, $messageVariableArr);
                }
            }
        }
        return $status;
