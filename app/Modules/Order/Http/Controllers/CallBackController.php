    ...
    public function wechatNotifyHandle($type, $data)
    {
        ...
        switch ($type){
            ...
            case 'employ':
                ...
                $status = false;
               if($order){
                    $res = UserDetailModel::recharge($order['uid'], 3, $data);
            ...
            case 'buy shop service':
                break;
            case 'vipshop':
                Log::info('进入回调5vipshop处理逻辑'.$data['code']);
                $waitHandle = VipshopOrderModel::where(['status' => 0, 'code' => $data['code']])->first();
                $status = false;
                if(!empty($waitHandle)){
                    $res = VipshopOrderModel::payVipShop($data,3);
                    if (is_null($res)){
                        $status = true;
                    }
                }else{
                ...
                Log::info('进入回调6vipshop处理成功'.$data['code']);
                break;
            case 'task service':
                $waitHandle = OrderModel::where('code', $data['code'])->first();
                $status = false;
