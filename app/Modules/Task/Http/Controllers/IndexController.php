...
class IndexController extends BasicIndexController
{
    ...
    public function create(Request $request)
    {
        ...
        if(!empty($category_all)){
            foreach($category_all as $k => $v){
                ...
                if($k == 0){
                    $category_second = $v['child_task_cate'];
                }
            }
        }
        ...
        $off = 100;
        if($userMore && $userMore['service_off'] > 0){
            $off = $userMore['service_off'];
        }
        $view = [
            'hotcate' => $hotCate,
    ...
    public function bountyUpdate(BountyRequest $request)
    {
        ...
        $taskModel = new TaskModel();
        $money = $taskModel->taskMoney($data['id'],$off);
    ...
    public function release($id,Request $request)
    {
        ...
        $pidCate = TaskCateModel::findById($task['cate_id']);
        if($pidCate){
            $pid = $pidCate['pid'];
            $category_second = TaskCateModel::findByPid([$pid]);
        }else{
            $pid = 0;
            $category_second = [];
        }
    ...
    public function postBuyServiceTaskBid(BountyRequest $request)
    {
        ...
        $service = array_flatten($service);
        $money = ServiceModel::serviceMoney($service,$off);
    ...
    {
        $pid = $request->get('id');
        $data = TaskCateModel::findByPid([$pid]);
