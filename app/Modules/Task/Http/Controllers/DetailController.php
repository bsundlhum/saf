...
class DetailController extends IndexController
{
	...
	public function index($id,Request $request)
    {
        ...
        if($detail->uid){
            $user = UserModel::find($detail->uid);
            $detail->user_name = $user->name;
            
            $inviteUser = TaskInviteModel::getUserByTaskId($id);
        }else{
            $detail->user_name = '';
            $inviteUser = [];
        }
        ...
        $category_all = $this->theme->get('task_cate');
        if(!empty($category_all)){
            foreach($category_all as $k => $v){
                if(!isset($v['pid']) || $v['pid'] != 0){
                    unset($category_all[$k]);
                }
            }
        }
        $province = DistrictModel::getSecondDistrict();
        $view = [
            'detail'             => $detail,
        ...
        $wechatShareImg = QrCode::size('280')->generate(url($request->path()));

        $view['wechat_share'] = $wechatShareImg;
    ...
    public function work($id)
    {
        ...
        $userMore = UserMoreModel::getUserMoreByUid(Auth::id());
        
        $workNum = WorkModel::where('uid',$this->user['id'])->whereIn('status',[0,1])->where('created_at','>=',date('Y-m-d 00:00:00'))->count();
        if($userMore && $userMore['task_work'] > 0 && $workNum >= $userMore['task_work']){
            return redirect()->back()->with('error','您今天的竞标次数不足，可以购买vip增加竞标次数');
        }
    ...
    public function workCreate(WorkRequest $request)
    {
        ...
        $userMore = UserMoreModel::getUserMoreByUid($this->user['id']);
        $workNum = WorkModel::where('uid',$this->user['id'])->whereIn('status',[0,1])->where('created_at','>=',date('Y-m-d 00:00:00'))->count();
        if($userMore && $userMore['task_work'] > 0 && $workNum >= $userMore['task_work']){
            return redirect()->back()->with('error','您今天的竞标次数不足，可以购买vip增加竞标次数');
        }
        ...
        if($task_delivery)
        {
            ...
            if($task_delivery->is_send_mobile == 1 && $task_delivery->code_mobile && $user->mobile){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                    $scheme => $task_delivery->code_mobile,
                ];
                $messageVariableArr = [
                    'username'   => $user['name'],
                    'name'       => $user_name,
    ...
    public function deliverCreate(WorkRequest $request)
    {
        ...
        if($agreement_documents)
        {
            ...
            if($agreement_documents->is_send_mobile == 1 && $agreement_documents->code_mobile && $user->mobile){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                    $scheme => $agreement_documents->code_mobile,
                ];
                $messageVariableArr = [
                    'username'=>$user['name'],
                    'initiator'=>$user_name,
    ...
    public function ajaxRights(Request $request)
    {
        ...
        if($trading_rights)
        {
            ...
            if($trading_rights->is_send_mobile == 1 && $trading_rights->code_mobile && $from_user->mobile){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                    $scheme => $trading_rights->code_mobile,
                ];

                \SmsClass::sendSms($from_user->mobile, $templates, $fromMessageVariableArr);
            }
        }
    ...
    public function report(Request $request)
    {
        ...
        if($task_publish_success)
        {
            ...
            if($task_publish_success->is_send_mobile == 1 && $task_publish_success->code_mobile && $this->user['mobile']){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                ...
                \SmsClass::sendSms($this->user['mobile'], $templates, $messageVariableArr);
            }
        }
    ...
    public function tenderWork($id)
    {
        ...
        $agree = AgreementModel::where('code_name','task_draft')->first();
        $min = \CommonClass::getConfig('bid_bounty_min_limit');
        $max = \CommonClass::getConfig('bid_bounty_limit');
        if($userMore && $userMore['most_task_bounty'] > 0 && $userMore['most_task_bounty'] < $max){
            $max = $userMore['most_task_bounty'];
        }
    ...
    public function ajaxInviteUser(Request $request)
    {
        ...
        if($request->get('Keyword')){
            $merge['service_name'] = $request->get('Keyword');
        }
        if($request->get('classifyArr')){
            $classifyArr = explode(',',$request->get('classifyArr'));
            if(isset($classifyArr[0])){
                $merge['pid'] = $classifyArr[0];
            }
            if(isset($classifyArr[1])){
                $merge['cate_id'] = $classifyArr[1];
            }
        }
        if($request->get('areaArr')){
            $areaArr = explode(',',$request->get('areaArr'));
            if(isset($areaArr[0])){
                $merge['province'] = $areaArr[0];
            }
            if(isset($areaArr[1])){
                $merge['city'] = $areaArr[1];
            }
        }
        $taskId = $request->get('task_id');
        $merge['task_id'] = $taskId;
        $inviteUser = TaskInviteModel::getUserByTaskId($taskId,6,$merge);
     ...
     public function inviteUser(Request $request)
     {
         ...
         if(!$task || $task->uid != $this->user['id'] || !in_array($task['status'],[3,4,5])){
             return $data = [
                 'code' => 0,
                 'msg'  => '参数错误'
             ];
