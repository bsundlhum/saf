...
Route::group(['prefix' => 'task','middleware' => ['auth','closeSite']], function() {
	...
	Route::get('/ajaxInviteUser','DetailController@ajaxInviteUser')->name('ajaxInviteUser');
	Route::get('/inviteUser','DetailController@inviteUser')->name('inviteUser');

});


Route::group(['prefix'=>'task','middleware' => ['closeSite']],function(){
	
	Route::get('/','IndexController@tasks')->name('taskList');
	
	Route::get('/{id}','DetailController@index')->name('taskDetailPage')->where('id', '[0-9]+');;
	Route::get('/getSecondCate','IndexController@getSecondCate')->name('getSecondCate');
