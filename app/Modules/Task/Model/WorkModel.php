    ...
    static public function workCheck($data)
    {
        ...
        if(is_null($status))
        {
            ...
            if($manuscript_settlement)
            {
                ...
                if($manuscript_settlement->is_send_mobile == 1 && $manuscript_settlement->code_mobile && $user->mobile){
                    $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                    $templates = [
                        $scheme => $manuscript_settlement->code_mobile,
                    ];
    ...
    static public function sendTaskWidMessage($arr)
    {
        ...
        if($task_win)
        {
            ...
            $messageVariableArr = [
                ...
                'win_price'   => number_format($task['bounty']/$task['worker_num'],2),
            ];
            ...
            if($task_win->is_send_mobile == 1 && $task_win->code_mobile && $user->mobile){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                    $scheme => $task_win->code_mobile,
                ];
                $messageVariableArr = [
                    'username'    => $user['name'],
                    'website'     => $site_name,
