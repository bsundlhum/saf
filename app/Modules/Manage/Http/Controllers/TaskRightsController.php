...
class TaskRightsController extends ManageController
{
    ...
    public function handleRights(Request $request)
    {
        ...
        if($trading_rights_result)
        {
            ...
            if($trading_rights_result->is_send_mobile == 1 && $trading_rights_result->code_mobile && $owner_user->mobile){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                    $scheme => $trading_rights_result->code_mobile,
                ];

                \SmsClass::sendSms($owner_user->mobile, $templates, $ownerMessageVariableArr);
            }
            ...
            if($trading_rights_result->is_send_mobile == 1 && $trading_rights_result->code_mobile && $worker_user->mobile){
                $scheme = ConfigModel::phpSmsConfig('phpsms_scheme');
                $templates = [
                    $scheme => $trading_rights_result->code_mobile,
                ];

                \SmsClass::sendSms($worker_user->mobile, $templates, $workerMessageVariableArr);
            }
        }
