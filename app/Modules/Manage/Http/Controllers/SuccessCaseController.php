...
class SuccessCaseController extends ManageController
{
    ...
    public function successCaseList(Request $request)
    {
        ...
        $orderByType = $request->get('orderByType') ? $request->get('orderByType') : 'asc';
        
        $page_size = $request->get('pageSize') ? $request->get('pageSize') : 10;
        ...
        $comments_page = $query->orderBy($orderBy,$orderByType)->paginate($page_size);
    ...
    {
        $data = $request->all();
        $user = UserModel::select('id','name')->get()->toArray();
