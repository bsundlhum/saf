...
class ConfigController extends ManageController
{
    ...
    public function saveConfigSite(Request $request)
    {
        ...
        $siteRule = array(
            ...
            'site_employer'   => isset($data['site_employer']) && !empty($data['site_employer']) ? $data['site_employer'] : '雇主',
            'site_employee'   => isset($data['site_employee']) && !empty($data['site_employee']) ? $data['site_employee'] : '威客',
            'site_close_desc'   => isset($data['site_close_desc']) && !empty($data['site_close_desc']) ? $data['site_close_desc'] : '很抱歉，您所访问的网站正在维护中'
        );
        ConfigModel::updateConfig($siteRule);
        Cache::forget('site');
        $basicRule = array(
            'css_adaptive' => isset($data['css_adaptive']) ? $data['css_adaptive'] : 0,
            'open_IM' => isset($data['open_IM']) ? $data['open_IM'] : 2,
            'qq' => isset($data['customer_service_qq']) ? $data['customer_service_qq'] : '',
            'IM_config' => json_encode(array(
                    'IM_ip' => isset($data['IM_ip']) ? $data['IM_ip'] : '',
                    'IM_port' => isset($data['IM_port']) ? $data['IM_port'] : '',
                )
            ),
        );
