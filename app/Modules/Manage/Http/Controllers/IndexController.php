...
class IndexController extends ManageController
{
    ...
    public function getManage()
    {
        ...
        $todayRecharge = FinancialModel::where('action', 3)->where('created_at', '>', $today)->where('created_at', '<', $tomorrow)->sum('cash');
        $yesterdayRecharge = FinancialModel::where('action', 3)->where('created_at', '>', $yesterday)->where('created_at', '<', $today)->sum('cash');
        ...
        $todayCashout = FinancialModel::where('action', 4)->where('created_at', '>', $today)->where('created_at', '<', $tomorrow)->sum('cash');
        $yesterdayCashout = FinancialModel::where('action', 4)->where('created_at', '>', $yesterday)->where('created_at', '<', $today)->sum('cash');
        if ($yesterdayCashout) {
    ...
    public function fileUpload(Request $request)
    {
        ...
        $result = json_decode(\FileClass::uploadFile($file,$relation_type),true);
        if(!is_array($result['data']) || $result['code'] != 200) {
            $result['status'] = 'failure';
        ...
        $html = AttachmentModel::getAttachmentHtml($data);
