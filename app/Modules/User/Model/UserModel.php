    ...
    CanResetPasswordContract
{
    ...
    protected $fillable = [
        ...
        'reset_password_code', 'remember_token','source','created_at','updated_at'
    ];
    ...
    public function initUser(array $data)
    {
        $status = DB::transaction(function() use ($data){
            ...
            UserMoreModel::create([
                'uid'               => $data['uid'],
                'task_work'         => in_array('TASK_WORK',array_keys($systemPackage)) ? $systemPackage['TASK_WORK']['rule'] : 0,
                'most_task_bounty'  => in_array('MOST_TASK_BOUNTY',array_keys($systemPackage)) ? $systemPackage['MOST_TASK_BOUNTY']['rule'] : 0,
                'skill_tags_num'    => in_array('SKILL_TAGS_NUM',array_keys($systemPackage)) ? $systemPackage['SKILL_TAGS_NUM']['rule'] : 0,
                'service_off'       => in_array('SERVICE_OFF',array_keys($systemPackage)) ? $systemPackage['SERVICE_OFF']['rule'] : 0,
